...
oneline_arr p,h,mid,z;        
FILE* fp;
int   sum_line=0;
char  acmd[20]={"displ"};     // record cmd//
char  bcmd[20]={"addl"};
char  ccmd[20]={"dell"};
char  dcmd[20]={"adds"};
char  ecmd[20]={"dels"};
...
int main(int argc,char**argv)
{
    ...
    for(j=0;j<wordnumber_of_eachline;j++)          //记录第一行 
    {
       if(((c=fgetc(fp))!='\n')&&(c!='\r')&&(c!='#')){h->context[j]=c;}
  ...
  //p=h;
 //for(i=0;i<=sum_line;i++){printf("%d--%s\n",p->line,p->context);p=p->next;}
//*********读入命令*********//
    ...
    for(i=0;i<N;i++)
    {
                    ...
                    else if(strcmp(rcmd,ccmd)==0)dell();
...
void     addl()
{
         ...
         while(((d=fgetc(fp))!='\n')&&(d!='\r')){read[i]=d;i++;}
                                                   //读取要插入的内容// 
         p=h;
         for(i=0;i<n;i++)p=p->next;    //指针移动要插入的那行// 
         if((z=(oneline_arr)malloc(sizeof(oneline)))==NULL)printf("the space is full\n");     
...
void     search()
{
         ...
         fgetc(fp);
         fgets(save,sizeof(save),fp);//printf("now search %s",save);
         l1=manage(save);
         save[l1-1]='\0';
         l1=manage(save);
         ...
         for(s=0;s<sum_line;s++)
         {
                                 ...
                                 //printf("%s\n",t_save);
                                 if(strstr(p->context,save)!=NULL)find++;
